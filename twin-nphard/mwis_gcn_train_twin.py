...
from gcn.layers import sparse_dropout_no_scale
from graph_util import consensus_matrix
# Settings (FLAGS)
from runtime_config import flags, FLAGS
from heuristics import *

flags.DEFINE_string('test_datapath', './data/ER_Graph_Uniform_GEN21_test1', 'test dataset')
...
for epoch in range(FLAGS.epochs):
    ...
    for id in np.random.permutation(len(val_mat_names)):
        ...
        f_ct += 1
        if FLAGS.architecture == 'decentralized':
            cmtx = consensus_matrix(adj_0)
            cons_mtcs.append(cmtx)
        if cnt < batch_size - 1:
            cnt += 1
            continue
        else:
        ...
        if FLAGS.architecture == 'decentralized':
            cons_block_diag = sp.block_diag(cons_mtcs*num_supports)
            dqn_agent.memory_t = list(zip(*dqn_agent.memory))
            gradients = dqn_agent.memory_t[0]
            gradients = list(zip(*gradients))  # [layer, batch, ...]
            for l, grad_l in enumerate(gradients):
                grad_l = tf.convert_to_tensor(grad_l)
                grad_l = tf.transpose(grad_l, (1, 0, 2, 3, 4))  # [kernel, batch, node, ...]
                grad_resh = tf.reshape(grad_l, (cons_block_diag.shape[0], -1))
                for _ in range(FLAGS.num_cons):
                    grad_resh = cons_block_diag @ grad_resh
                grad_resh *= FLAGS.num_nodes
                grad_resh = tf.reshape(grad_resh, grad_l.shape)
                grad_l = tf.unstack(grad_resh, axis=1)
                gradients[l] = grad_l
            gradients = list(zip(*gradients))  # [batch, layer, ...]
            dqn_agent.memory_t[0] = gradients
            dqn_agent.memory = list(zip(*dqn_agent.memory_t))
